...
function OrderCardView({ order, onClick }: { order: Order; onClick: () => void }) {
  ...
  return (
    <div className="rounded-lg bg-card border border-border/60 p-4 md:p-5 space-y-3 hover:border-foreground transition-colors cursor-pointer" onClick={onClick}>
      ...
      {order.status === "CREATED" && (
        <div className="flex justify-center gap-2 pt-1">
          ...
          <Button size="sm" variant="outline" className="flex-1 h-8 text-xs gap-1.5 text-destructive hover:text-destructive" onClick={e => e.stopPropagation()}>
